...
type Connection struct {
	...
	//当前Conn属于哪个Server
	UDPServer _interface.Server //当前conn属于哪个server，在conn初始化的时候添加即可
...
func NewConnection(server _interface.Server, conn *net.UDPConn, connID uint32, msgHandler _interface.MsgHandle) *Connection {
	c := &Connection{
		UDPServer:   server,
		Conn:        conn,
		ConnID:      connID,
	...
	c.GetUDPServer().GetConnMgr().Add(c) //将当前新创建的连接添加到ConnManager中
	return c
}
...
func (c *Connection) GetUDPServer() _interface.Server {
	return c.UDPServer
}
...
func (c *Connection) startReader() {
	...
	for {
		...
		default:
			// 读取客户端 Msg 的头
			buf := make([]byte, 256)
			udp, _, err2 := c.Conn.ReadFromUDP(buf)
			if err2 != nil {
				fmt.Println(err2)
			}
			// 拆包 得到 MsaID 和 MsgDataLen
			msg, err := c.UDPServer.Packet().UnPack(buf)
			if err != nil {
				fmt.Println("解包 异常", err)
				continue
			}
			if udp <= c.UDPServer.Packet().GetHeadLen() {
				break
			}
			msg.SetMsgData(buf[c.UDPServer.Packet().GetHeadLen():udp])
...
func (c *Connection) finalizer() {
	...
	//将链接从连接管理器中删除
	c.UDPServer.GetConnMgr().Remove(c)
